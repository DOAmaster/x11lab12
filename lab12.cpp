...
void newinit(void)
{
	...
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, g.ball2Y, g.ball2Z, o->center);
	o->specular = true;
	...
	g.nobjects++;
	//--------------------------------------------------------------------
	//sphere sitting on floor 2
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, g.ballY, g.ballZ, o->center);
	o->specular = true;
	vecMake(0.7, 0.7, 0.7, o->spec);
	...
	vecMake(0,0,1, g.sky[1]);
	g.nlights=0;
	vecMake(-400, 600, 500, g.lightPos[g.nlights]);
	vecMake(.4, .4, .4, g.lightColor[g.nlights]);
	++g.nlights;
	vecMake(400, 600, 550, g.lightPos[g.nlights]);
	vecMake(.4, .4, .4, g.lightColor[g.nlights]);
	++g.nlights;
	vecMake(-300, 800, 200, g.lightPos[g.nlights]);
	vecMake(.2,.2,.2, g.lightColor[g.nlights]);
	++g.nlights;
	vecMake(g.camX, g.camY, 800.0, g.from);
...
void init(void)
{
	...
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, 40.0, 340.0, o->center);
	o->specular = true;
	vecMake(0.7, 0.7, 0.7, o->spec);
	vecMake(.2, .2, .1, o->color);
	o->radius = 40.0;
	...
	vecMake(.2,.2,.2, g.lightColor[g.nlights]);
	++g.nlights;
	vecMake(160.0, 160.0, 800.0, g.from);
	vecMake(0.0, 80.0, 0.0, g.at);
	vecMake(0.0, 1.0, 0.0, g.up);
	g.angle = 45.0;
}
...
void lab12_animation(int nframes)
{
    ...
    newinit();
		...
		//take a screen-shot
    takeScreenshot("", "", 0);
    //camera moves until the small sphere comes close to large sphere
    if ( g.ballZ < 100.0) {
